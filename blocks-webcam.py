...
def main():
    ...
    while(True):
        ...
        window_scale = float(frame.shape[0]) / float(frame.shape[1])
        window_width = VIEW_WIDTH
        window_height = int(window_width * window_scale)
        #img = cv2.resize(frame, (window_width, window_height))
        img = frame
        ...
        for cnt in contours:
            ...
            if (min(b[0]) >= ROI_XY[0] and min(b[0]) <= ROI_XY[2]  and \
               ...
               (contour_area >= 1500 and contour_area <= 2500):
                ...
                contour_text = '#{}: ({:.1f},{:.1f}) [mm], {:.0f} [deg], {}'.format(i, x, y, contour_rotation, box_color)
                cv2.putText(img, contour_text, (px, py), font, 0.3, (255, 255, 255), 1, cv2.LINE_AA)
        ...
        cv2.putText(img, 'Mode: ?', (1, 15), font, 0.4, (255, 255, 255), 1, cv2.LINE_AA)
        cv2.putText(img, fps_text, (1, 30), font, 0.4, (255, 255, 255), 1, cv2.LINE_AA)
        #cv2.rectangle(img, (ROI_XY[0],ROI_XY[1]), (ROI_XY[2],ROI_XY[3]), (255,0,0), 1)
